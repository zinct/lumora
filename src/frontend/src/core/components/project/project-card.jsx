...
export function ProjectCard({ project, onClick }) {
  const avatarUrl = useMemo(
    () =>
      `https://api.dicebear.com/7.x/identicon/svg?seed=${project.title}&backgroundColor=c7d2fe&shape1Color=4f46e5`,
    [project.title]
  );

  const coverImageUrl = useMemo(
    () =>
      `https://api.dicebear.com/7.x/shapes/svg?seed=${project.title}&backgroundColor=c7d2fe,ddd6fe,fae8ff,dbeafe,bfdbfe,e0e7ff&shape1Color=4f46e5,6d28d9,7c3aed,2563eb,3b82f6,4f46e5`,
    [project.title]
  );

  return (
    <Card
      className="overflow-hidden bg-card border-border/40 cursor-pointer hover:border-border/60 transition-colors"
      onClick={onClick}
    >
